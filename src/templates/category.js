...
export const Head = ({ pageContext }) => (
  <SEO
    title={`${capitalize(pageContext.category)}`}
    path={`blog/category/${pageContext.category}/`}
  />
)
  ...
  return (
    <Layout>
      <div className={`${utilStyles.mb1} ${utilStyles.jcenter}`}>
        <h1 className={`${utilStyles.doubleSize}`}>
          {capitalize(pageContext.category)} Posts
        </h1>
      </div>
        ...
        <ol className={styles.posts}>
          {data.allContentfulBlogPost.edges.map(({ node }) => {
            ...
            if (node.description) {
              ...
              desc =
                "If you would like to read this post, please follow the post link!"
            }
            const { title, slug, publishedDate, category } = node
            return (
              <>
                <li
                  className={`${utilStyles.mtb2} ${styles.post}`}
                  key={node.id}
                >
